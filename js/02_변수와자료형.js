...
function varCheck(){
    ...
    // 값 변경 확인 ("var"라는 키워드를 작성하지 않음)
    menu = "라면";
    console.log("menu : ", menu);

    //변수명 중복 확인 ("var"라는 키워드 O)
    var menu = "김밥";
    console.log("menu : ", menu);
...
{ // 함수 정의
    ...
    // if( num3 === 300 ) : 만약에 num3과 300이 같다면 -> {}블럭 수행
    if( num3 === 300 ){
...
{    // 함수 정의
    ...
    console.log("if 전 : ", name); // 결과 : 홍길동

    if(temp === 123){
        const name = "김미영"; // {} 바깥쪽, 안쪽 변수명이 같으면
                               // {} 안쪽이 우선 순위가 높음!!
        console.log("if 중 : ", name); // 결과 : 김미영

    }

    console.log("if 후 : ", name); // 결과 : 홍길동
}
...
function typeCheck(){
    ...
    /* undefined : 정의되지 않음 == 변수의 값이 들어지가 않음 X */
    let undef;
    ...
    // JS 객체 내 요소 하나씩 꺼내기
    console.log("user['id'] : ", user['id'], typeof user['id']); // string 타입
    console.log("user['pw'] : ", user['pw'], typeof user['pw']); // string 타입
    console.log("user['point'] : ", user['point'], typeof user['point']); //number 타입
    ...
    console.log("sumFn : ", sumFn, typeof sumFn); // 함수 타입

    // sumFn 호출
    console.log( sumFn(111, 987) );
    console.log( sumFn(1234, 5678) );
