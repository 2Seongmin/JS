...
btn1.addEventListener("click", () => {
    ...
    input1.setAttribute("type", "checkbox");    // type 속성의 값을 "checkbox"로 설정(추가) -> checkbox로 변경됨
    input1.setAttribute("class", "big");        // class 속성의 값을 big으로 설정
    /* JS 속성 설정 방법 중 Class 관련
        + CSS의 class 기반 설계를 이용해서 HTML 요소의 스타일을 변경할 수 있다. */
...
btn2.addEventListener("click", () =>  {
    ...
    /* [ tip ] */
    // 콘솔에서 특정 요소가 가지고 있는 DOM 속성을 직접 확인하고 싶을 경우
    // -> 복수 형태로 요소를 얻어오는 querySelectorAll(), getElementsByClassName() 등을 이용
...
addBtn.addEventListener("click", () => {
    ...
    console.log(btnContianer.classList.contains("hidden"));

    // .btn-container에 hidden 클래스가 포함되어 있을 경우
    // == 화면에 안보이는 경우
    if(btnContianer.classList.contains("hidden")){
        
        // 요소.classList.remove("클래스1" [, ...])
        // -> 클래스 제거
        btnContianer.classList.remove("hidden");
...
closeBtn.addEventListener("click", () => {
    ...
    if( !btnContianer.classList.contains("hidden") ){
        btnContianer.classList.add("hidden");
...
/* 요소.style.css속성 방법은 inline-style로 적용되기 때문에 
...
btns.forEach((btn) => {
    ...
    btn.addEventListener("click", () => {
        ...
        // = container3의 클래스 모두 지우고 container-3 클래스를 추가
