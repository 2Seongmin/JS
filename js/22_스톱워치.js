...
startBtn.addEventListener("click", () => {
    ...
    // START <-> PAUSE 버튼으로 변경
    if(startBtn.innerText === "PAUSE"){  // 버튼이 PAUSE 일 경우
        startBtn.innerText = "START";
        recordBtn.disabled = true; // 비활성화

        // setInterval() 을 깨끗이 치워버림
        // -> clearInterval(); 이용
        clearInterval(currentInterval) // setInterval() 동작 멈춤
...
function displayOutput(){
    ...
    const second = attachZero( Math.floor(count / 100 % 60) );  // 100으로 나눈 몫 의 60으로 나눈 나머지
    const ms = attachZero( count % 100 );   // 100으로 나눈 나머지
...
recordBtn.addEventListener("click", () => {
    ...
    // console.log(display.innerHTML);  // 태그까지 모두 얻어옴 -> X
