...
function check9(){
    ...
    for(let num = 3; num <= 30; num++){   // num 1씩 증가
        ...
        if(num % 3 === 0){   // 3으로 나눴을 대 나머지 0 == 3의 배수
            result2 += num + " ";
        }
    }
    console.log(result2);
...
function check10(){
    ...
    let result1 = "";  // 배수를 한 줄로 출력할 변수
    let sum = 0        // 합계를 저장할 변수

    for(let num = start; num <= end; num++){
        if(num % multiple === 0){
            result1 += num + " ";  // 출력 문자열 누적
            sum += num;            // 합계 누적
        }
    }
    console.log(result1);
...
/** 입력 받은 단의 구구단 출력하기 */
function check12(){
    const val = document.getElementById("dan12");
...
// 단, 취소 입력 시 추가 입력을 받지 않고 누적된 합계를 alert()로 출력
function check28(){
