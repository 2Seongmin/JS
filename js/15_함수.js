...
// 버튼 클릭 시 prompt를 이용해서 정수, 지수 입력받아 결과값 출력
...
/* Closure(클로저)
    ...
    특징 2 : 상태 유지
     - 외부에서 함수를 수행한 후에도 내부 함수의 변수 값이 변한 상태로 유지됨
*/
...
arrows[1].addEventListener("click", () => {
    ...
    // 2 제곱 반환
    const square = num => {    // () 생략 (매개변수 1개일 경우)
        return num * num;
    }
...
arrows[2].addEventListener("click", () => {
    ...
    const print = (str) => console.log(str);    // {}  생략 (구문이 한 줄인 경우)

    print("안녕하세요.");

    const doubleFn = num => num * 2;          // return, {} 생략 + () 생략
...
const str = "A";  // 전역 변수
console.log(str);  // A 출력

(function(){
    const str = "B" // {} 내 지역 변수

    console.log(str)   // B 출력
})();