...
function check4(){
    ...
    for(let i=0; i<arr.length; i++){
        sum += arr[i];  // i번재 인덱스 요소를 얻어와 sum에 누적
    }
    console.log("합계 : ", sum);
    ...
    /* 짝수번째(0,2,4) index 요소의 합계 출력 */
    /* 홀수번째(1,3) index 요소의 합계 출력 */
...
function createLottoNumber(){
    ...
    for(let i=0; i<6; i++){
        ...
        if(arr.includes(num)){
            ...
            /* 이 구문이 없으면 for문의 i가 계속 증가해서 비어있는 배열 요소가 발생 */
            // index를 뒤로 갔다, 앞으로 오게 함 == 제자리 유지
            i--;

            continue; // 밑의 명령어를 실행시키지 않고, 다음 반복으로 넘어감
        }
...
/* 4행 4열짜리 2차원 배열에 1~16 사이 난수 배치 (중복 X) 
    1) 배열 콘솔에 출력
    2) prompt를 이용해서 1~16 사이 숫자를 입력 받아 2차원 배열 어디에 위치하는지 검색해서 출력
*/
function check7(){

    // 중복 없이 난수 16개를 저장한 1차원 배열 생성
    const randomArr = [];

    for(let i = 0; i < 16; i++){
        
        const num = Math.floor(Math.random() * 16 + 1);  // 1 ~ 16 사이의 난수
    ...
    for(let row = 0; row < 4; row++){  // 행 반복
        ...
        for(let col = 0; col < 4; col++){ // 열 반복
            ...
            index++; // index를 1 증가시켜 randomArr의 다음 요소 지정하기
        }
    }
    console.log(arr);
