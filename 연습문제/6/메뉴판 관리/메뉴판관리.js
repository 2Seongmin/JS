...
/* #resName (span)이 클릭된 경우 
    #resName을 숨기고 #resNameInput을 보이게 만들기 */
...
/* #resNameInput이 포커스를 잃었을 때(blur) 
    #resName을 보이게 만들고 #resNameInput을 숨기기 */
...
updateBtn.addEventListener("click", () => {
    ...
    menus.forEach(menu => {
        ...
        const menuName = menu.children[0].innerText.replace('원', '');
        const menuPrice = menu.children[1].innerText.replace('원', '');

        // 5) .menu 내부에 추가될 input객체를 모아둔 JS객체 만들기
...
exitBtn.addEventListener("click", () => {
    ...
    menus.forEach(menu => {
        ...
        const menuName = menu.children[1].value;
        const menuPrice = menu.children[2].value;
